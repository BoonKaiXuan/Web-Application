...
/* Create an array to store Game 2 counts for different emails */
if (!isset($_SESSION['game2_play_count'])) {
    $_SESSION['game2_play_count'] = [];
}

/* First time this email plays Game 2 */
if (!isset($_SESSION['game2_play_count'][$email])) {
    $_SESSION['game2_play_count'][$email] = 0;
}

if (isset($_POST['btn_no'])) {
    $selected_no = (int)$_POST['btn_no'];

    if ($_SESSION['game2_play_count'][$email] >= 2) {
        $_SESSION['message'] = "Limit reached! You cannot submit more than 2 times.";
        header("Location:game2.php");
        exit();
    } else {
        $sql = "UPDATE game SET G2 = '$selected_no' WHERE email = '$email'";

        if ($conn->query($sql) === TRUE) {
            $_SESSION['game2_play_count'][$email]++;
            $_SESSION['message'] = "Updated! Play count: " . $_SESSION['game2_play_count'][$email] . "/2";
            header("Location:game2.php");
            exit();
        }
    }
