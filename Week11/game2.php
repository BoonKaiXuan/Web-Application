...
if (isset($_POST['btn_no'])) {
    ...
    $click_query = "SELECT G2_click FROM game WHERE UID = '$uid'";
    ...
    if ($click_result && $click_result->num_rows > 0) {
        ...
        $curr_click = (int)$click_row['G2_click'];

        if ($curr_click >= 2) {
            $message = "Limit reached! You cannot submit more than 2 times.";
        } else {
            $sql = "UPDATE game SET G2 = '$selected_no', G2_click = $curr_click +1 WHERE UID = '$uid'";
